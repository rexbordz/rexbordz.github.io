...
(function () {
  ...
  var CACHE_KEY = 'rexbordz:search:v2:' + stamp();
  var index = null;
  var loading = null;
  ...
  function render() {
    if (!results.length) {
      list.innerHTML = '<div class="sr-empty">Nothing matches that.</div>';
      return;
    }
    var last = null, html = '';
    results.forEach(function (r, i) {
      ...
      html += '<a class="sr-item' + (i === cursor ? ' is-on' : '') + '" data-i="' + i + '" href="' + C.esc(r.href) + '">' +
        ...
        '<span class="sr-sub">' + C.esc(r.kind === 'widget' ? r.sub : 'docs') + '</span></a>';
    });
    list.innerHTML = html;
      ...
      '<div class="search-modal" role="dialog" aria-label="Search">' +
        '<div class="search-panel">' +
          '<div class="search-field">' +
            ...
            '<input type="text" placeholder="Search widgets and docs" autocomplete="off" spellcheck="false">' +
            '<kbd>esc</kbd>' +
          '</div>' +
